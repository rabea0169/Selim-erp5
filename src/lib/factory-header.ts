...
export function buildFactoryHeader(settings: FactorySettings): string {
  return `
    <div style="text-align: center; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
      ${settings.logo
        ? `<img src="${settings.logo}" style="max-height: 80px; max-width: 180px; margin: 0 auto 8px; display: block;" />`
        : ''
      }
      <h1 style="margin: 0; font-size: 20px; color: #0f172a; font-weight: bold;">${settings.factoryName}</h1>
      ${settings.factoryNameEn ? `<p style="margin: 2px 0; font-size: 12px; color: #64748b;">${settings.factoryNameEn}</p>` : ''}
      ${settings.slogan ? `<p style="margin: 4px 0; font-size: 11px; color: #475569;">${settings.slogan}</p>` : ''}
      <div style="margin-top: 8px; font-size: 10px; color: #64748b;">
        ${settings.phone ? `<span>📞 ${settings.phone}</span>` : ''}
        ${settings.address ? ` <span style="margin: 0 8px;">|</span> <span>📍 ${settings.address}</span>` : ''}
        ${settings.email ? ` <span style="margin: 0 8px;">|</span> <span>✉️ ${settings.email}</span>` : ''}
      </div>
      ${(settings.taxNumber || settings.commercialRegister) ? `
        <div style="margin-top: 4px; font-size: 10px; color: #64748b;">
          ${settings.taxNumber ? `<span>س.ض: ${settings.taxNumber}</span>` : ''}
          ${settings.commercialRegister ? ` <span style="margin: 0 8px;">|</span> <span>س.ت: ${settings.commercialRegister}</span>` : ''}
        </div>
      ` : ''}
    </div>
  `
...
export function buildFactoryFooter(settings: FactorySettings): string {
  return `
    <div style="margin-top: 24px; padding-top: 12px; border-top: 1px dashed #94a3b8; text-align: center; font-size: 10px; color: #64748b;">
      <p>${settings.invoiceFooter || 'شكراً لتعاملكم معنا'}</p>
