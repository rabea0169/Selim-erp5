      ...
      if (data.expenseCategories?.length) {
        for (const c of data.expenseCategories) {
          await tx.expenseCategory.create({
            data: {
              id: c.id, name: c.name, notes: c.notes ?? null,
              createdAt: new Date(c.createdAt), companyId: auth.companyId,
            },
          })
        }
      }
        ...
        for (const e of data.expenses) {
          await tx.expense.create({
            data: {
              ...
              notes: e.notes ?? null, createdAt: new Date(e.createdAt), companyId: auth.companyId,
            },
          })
        }
      }
      if (data.workers?.length) {
        for (const w of data.workers) {
          await tx.worker.create({
            data: {
              id: w.id, name: w.name, phone: w.phone ?? null, job: w.job ?? null,
              type: w.type || 'monthly', notes: w.notes ?? null,
      ...
      if (data.customers?.length) {
        for (const c of data.customers) {
          await tx.customer.create({
            data: { id: c.id, name: c.name, phone: c.phone ?? null, address: c.address ?? null, notes: c.notes ?? null, createdAt: new Date(c.createdAt), companyId: auth.companyId },
          })
        }
      }
      if (data.suppliers?.length) {
        for (const s of data.suppliers) {
          await tx.supplier.create({
            data: { id: s.id, name: s.name, phone: s.phone ?? null, address: s.address ?? null, notes: s.notes ?? null, createdAt: new Date(s.createdAt), companyId: auth.companyId },
          })
        }
      }
      if (data.sales?.length) {
        for (const s of data.sales) {
          await tx.sale.create({
            data: { id: s.id, invoiceNo: s.invoiceNo ?? null, customerName: s.customerName, customerId_ref: s.customerId_ref || null, date: new Date(s.date), total: Number(s.total), paid: Number(s.paid), notes: s.notes ?? null, createdAt: new Date(s.createdAt), updatedAt: new Date(s.updatedAt), companyId: auth.companyId },
          })
        }
      }
      ...
      if (data.purchases?.length) {
        for (const p of data.purchases) {
          await tx.purchase.create({
            data: { id: p.id, invoiceNo: p.invoiceNo ?? null, supplierName: p.supplierName, supplierId_ref: p.supplierId_ref || null, date: new Date(p.date), total: Number(p.total), paid: Number(p.paid), notes: p.notes ?? null, createdAt: new Date(p.createdAt), updatedAt: new Date(p.updatedAt), companyId: auth.companyId },
          })
        }
      }
