  ...
  const handleRestore = async () => {
    ...
    try {
      const res = await fetch('/api/restore', {
        ...
        body: JSON.stringify({ data: confirmRestore.data }),
      }).then((r) => r.json())
      if (res.error) throw new Error(res.error)
      toast({
        title: 'تم الاسترجاع',
        description: `تم استرجاع البيانات بنجاح (${res.counts.workers} عامل، ${res.counts.customers} عميل، ${res.counts.sales} مبيعة)`,
      })
        ...
        <div className="space-y-3">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-800">
            <div className="flex items-start gap-2">
              ...
              <p>
                يتم حفظ كل بياناتك محلياً على الجهاز. يُنصح بعمل نسخة احتياطية بشكل دوري وحفظها على
                جهازك أو إرسالها عبر الواتساب لحفظها.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-3">
            ...
            <p className="text-[11px] text-slate-500 mb-2">
              يحفظ كل البيانات (عمال، عملاء، موردين، مبيعات، مشتريات، مصاريف...) في ملف JSON
            </p>
            <Button
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-3">
            ...
            <p className="text-[11px] text-slate-500 mb-2">
              استرجاع البيانات من ملف نسخة احتياطية سابق. سيتم استبدال كل البيانات الحالية!
            </p>
            <input
              ref={fileRef}
              type="file"
          ...
          <div className="bg-rose-50 border border-rose-100 rounded-lg p-2 text-[11px] text-rose-700">
            <div className="flex items-start gap-1">
              ...
              <p>الاسترجاع سيحذف كل البيانات الحالية ويستبدلها ببيانات الملف. تأكد من عمل نسخة قبل الاسترجاع.</p>
            </div>
          </div>
        </div>
      </DialogContent>
